...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:12345678@localhost/stock_vetement'
app.config['SECRET_KEY'] = 'fashion-maroc-secret-key-2024'
...
def dashboard():
    ...
    alertes = Stock.query.filter(Stock.quantite_disponible < Stock.seuil_minimum).all()
    commandes_en_cours = Commande_Fournisseur.query.filter_by(statut='EN_COURS').count()
    commandes_recentes = Commande_Fournisseur.query.order_by(Commande_Fournisseur.date_commande.desc()).limit(5).all()
...
@app.route('/produits')
def produits():
    tous_produits = Produit.query.order_by(Produit.id_categorie).all()
    return render_template('produits.html', produits=tous_produits)

@app.route('/commandes')
def commandes():
    toutes_commandes = Commande_Fournisseur.query.order_by(Commande_Fournisseur.date_commande.desc()).all()
    return render_template('commandes.html', commandes=toutes_commandes)

@app.route('/fournisseurs')
def fournisseurs():
    tous_fournisseurs = Fournisseur.query.all()
    return render_template('fournisseurs.html', fournisseurs=tous_fournisseurs)
